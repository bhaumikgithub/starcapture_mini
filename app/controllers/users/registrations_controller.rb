...
class Users::RegistrationsController < Devise::RegistrationsController
  ...
  def create
    ...
    if (session[:trip] && resource.present?)
      ...
      trip_dup = trip.dup
      trip_dup.user_id = resource.id
      trip_dup.save!
      trip.trip_schedules.sort_by(&:created_at).each do |trip_schedule|
        ts = trip_dup.trip_schedules.new(trip_schedule.attributes.except('id'))
        ts.created_at = Time.now
